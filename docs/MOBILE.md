...
## Export JSON prin dialogul nativ
Setări folosește `@capacitor/filesystem` 8.1.3 și `@capacitor/share` 8.0.2. Se scrie o copie în cache-ul privat `medical-exports/<timestamp-uuid>/export.json`, apoi se deschide dialogul sistemului, fără alegerea automată a unei aplicații destinatare. Android FileProvider expune numai acest subdirector. Nu se cer permisiuni generale pentru stocare externă. Fișierul temporar este necriptat pentru a putea fi citit de aplicația aleasă; utilizatorul inițiază explicit exportul. Nu se șterge imediat la închiderea dialogului, deoarece destinatarul poate citi ulterior.
La următorul export se șterg numai directoarele proprii mai vechi de 24 de ore. Închiderea aplicației nu garantează ștergerea exact la 24 de ore; sistemul poate curăța cache-ul. Copiile salvate/trimise de utilizator nu sunt controlate de aplicație. Limita nativă este 25 MB; fișierele mai mari necesită versiunea web. Anularea/eroarea nu este prezentată drept salvare reușită. Descărcările PDF/DOCX și originalele folosesc încă mecanismul web și necesită conectarea la adaptorul comun într-un increment următor.
