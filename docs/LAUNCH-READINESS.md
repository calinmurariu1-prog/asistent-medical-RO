...
Cache API: middleware-ul aplică no-store, Pragma no-cache și Expires 0 tuturor răspunsurilor din prefixul API, inclusiv profil, tokenuri, erori de validare și rute inexistente. Clientul fetch păstrează no-store. 14 teste securitate/sesiuni/arhivă trecute și Ruff trecut. Aceasta împiedică reutilizarea prin cache HTTP conform antetelor; nu certifică ștergerea fișierelor descărcate sau a capturilor făcute de utilizator.
