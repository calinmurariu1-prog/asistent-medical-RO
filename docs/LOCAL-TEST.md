...
`export_metadata` declară explicit acoperirea. Originalele nu sunt încorporate în JSON; fiecare document oferă o cale de descărcare care necesită autentificarea proprietarului. CNP, facturarea și auditul nu sunt încă incluse. Măsurătorile de sănătate, dispozitivele raportate, notificările și feedbackul propriu sunt incluse. Tokenurile push și marcajele interne de livrare nu sunt exportate. Secretele de autentificare și cheile interne de stocare nu se exportă. Răspunsul are `Cache-Control: no-store`; fișierul descărcat rămâne în responsabilitatea utilizatorului. Acest increment nu reprezintă un pachet integral de portabilitate și nu certifică conformitatea juridică GDPR.
