...
`export_metadata` declară explicit acoperirea. Originalele nu sunt încorporate în JSON; fiecare document oferă o cale de descărcare care necesită autentificarea proprietarului. Exportul JSON obișnuit omite CNP; opțiunea din Setări îl include numai după reconfirmarea parolei și a celui de-al doilea factor când MFA este activ. Facturile externe nu sunt incluse. Istoricul acțiunilor proprii este inclus în `audit_events`, cu dată, resursă, IP și user-agent; textul liber intern `detail` este exclus explicit din export. Evenimentele altor conturi sau fără proprietar nu sunt incluse. Măsurătorile de sănătate, dispozitivele raportate, notificările și feedbackul propriu sunt incluse. Tokenurile push și marcajele interne de livrare nu sunt exportate. Secretele de autentificare și cheile interne de stocare nu se exportă. Răspunsul are `Cache-Control: no-store`; fișierul descărcat rămâne în responsabilitatea utilizatorului. Acest increment nu reprezintă un pachet integral de portabilitate și nu certifică conformitatea juridică GDPR.
