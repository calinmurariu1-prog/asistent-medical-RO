...
Exportul JSON se descarcă din **Setări → Exportă datele din dosar → Descarcă dosarul JSON**. Butonul folosește sesiunea autentificată și mecanismul de reînnoire existent, afișează erori cu posibilitate de reîncercare și precizează că inițierea descărcării nu confirmă salvarea pe disc. Numele fișierului conține doar data, fără numele pacientului. Testele de browser verifică efectiv JSON-ul descărcat și identitatea contului pe cele trei dimensiuni de ecran. În buildurile native, controlul este momentan dezactivat cu explicație: adaptorul nativ pentru salvare/partajare rămâne de implementat. Browserul mobil și PWA folosesc descărcarea web.
